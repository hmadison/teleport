...
Sometimes you may want to reset the `teleport` process to a clean state. This
can be accomplished by erasing everything under the `"data_dir"` directory.
Assuming the default location, `rm -rf /var/lib/teleport/*` will do.
