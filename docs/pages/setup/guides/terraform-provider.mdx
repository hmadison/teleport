...
```code
$ tctl auth sign --user=terraform --out=auth.pem
```
...
<Notice type="tip" scope={["oss", "enterprise"]}>

Replace `proxy.example.com:443` with the address of your Teleport Auth Service
or Proxy Service.

</Notice>
<Notice type="tip" scope={["cloud"]}>

Replace `teleport.example.com:443` with the address of your Teleport Cloud
tenant, e.g., `mytenant.teleport.sh`.
