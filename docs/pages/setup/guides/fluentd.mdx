...
The Fluentd Teleport plugin uses the `fluentd` role and user to read events. We
export an identity file for the user with the `tctl auth sign` command.
...
This will generate a file called `identity` that contains TLS certificates and
will be used to connect the Fluentd plugin to your Teleport instance.
