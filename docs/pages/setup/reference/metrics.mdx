...
Teleport provides HTTP endpoints for monitoring via Prometheus. They are
disabled by default, but you can enable them by running `teleport start` using
the `--diag-addr` flag:
...
- `http://127.0.0.1:3000/readyz` is similar to `/healthz`, but it returns "OK"
  *only after* the Teleport instance successfully joins a cluster, i.e., it draws the
