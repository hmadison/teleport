...
In this guide, we will set up an Ansible playbook to run the OpenSSH client
...
In particular, you will be using `/opt/machine-id/ssh_config` file in your
Ansible configuration to define how Ansible should connect to Teleport Nodes.

## Step 1/2. Configure Ansible

Create a folder `ansible` where all Ansible files will be collected.
...
You can create an inventory file `hosts` manually or use a script like the one
