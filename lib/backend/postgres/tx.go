...
func (tx *pgTx) DeleteEvents(beforeEventID int64) {
	if tx.err != nil {
		return
	}

	const query = `DELETE FROM event WHERE eventid <= $1`
	_, err := tx.sqlTx.ExecContext(tx.ctx, query, beforeEventID)
	tx.rollback(err)
}
...
func (tx *pgTx) DeleteItems(beforeEventID int64) {
	if tx.err != nil {
		return
	}
	...
	const query = `
		DELETE FROM item WHERE (key, id) NOT IN (
			...
			UNION
			SELECT key, id FROM event WHERE eventid >= $2
		)`
	_, err := tx.sqlTx.ExecContext(tx.ctx, query, tx.now(), beforeEventID)
	tx.rollback(err)
}
...
func (tx *pgTx) GetEvents(fromEventID int64, limit int) (lastEventID int64, events []backend.Event) {
	if tx.err != nil {
		return fromEventID, nil
	}

	// Return fromEventID when the query returns zero events.
...
func (tx *pgTx) InsertEvent(eventType types.OpType, item backend.Item) {
	...
	const query = `INSERT INTO event (key, id, type) VALUES ($1,$2,$3)`
	_, err := tx.sqlTx.ExecContext(tx.ctx, query, item.Key, item.ID, eventType)
	tx.rollback(err)
}
